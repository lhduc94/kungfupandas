...
Khi bạn sử dụng Python version >= 3.6 và pandas version >= 0.23 và không truyền biến `index` vào, index của `Series` sẽ vẫn giữ nguyên theo thứ tự index của dict truyền vào. 

Khi bạn sử  dụng pandas version <= 0.23 và không truyền biến `index` vào,  `Series` sẽ đánh index là index của dict được sắp xếp theo thứ tự từ vựng.
``` 
Ví dụ: với bản pandas >= 0.23, index của `Series` sẽ là `["b", "a", "c"]`, còn nếu bạn dùng phiên bản thấp hơn thì index của `Series` sẽ là `["a", "b", "c"]` 
...
```python
In [1]: pd.Series({"a":0, "b":1, "c": 2}, index=["b", "c", "d", "a"])
Out[1]: 
b    1
c    2
d    NaN
