...
index của DataFrame sẽ được tạo từ `0` đến `len(n) - 1` trong đó `n` là độ dài của mảng. 
```python
In [1]: d = {
            "one": [1, 2, 3, 4],
            "two": [1, 2, 3, 4]
...
```python
In [1]: pd.DataFrame(data=[[1, 2], [3, 4, 5]], index=["a", "b"], columns=['one','two','three'])
Out[1]: 
   one  two  three
a    1    2    NaN
b    3    4    5.0

In [2]: pd.DataFrame(data=np.random.rand(2,3), index=["a", "b"], columns=['one','two','three']))
Out[2]:
        one       two     three
...
**Khởi tạo DataFrame từ bản ghi có cấu trúc**
