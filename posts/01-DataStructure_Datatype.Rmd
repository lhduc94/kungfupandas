...
Mục tiêu của chương này là giới thiệu về các cấu trúc cơ bản trong Pandas là `Series` và `DataFrame`. 
Trong chương này, bạn sẽ học cách khởi tạo các cấu trúc này cũng như một số thao tác cơ bản trên `Series`. 
Bạn cũng sẽ được biết về một số kiểu dữ liệu thường dùng trong pandas và cách để giảm thiểu bộ nhớ sử dụng khi khởi tạo dữ liệu.
...
Để kiểm tra kiểu dữ liệu của `Series` hay `DataFrame` bạn có thể gọi thuộc tính `dtypes`. Các kiểu dữ liệu thường gặp của Pandas được mô tả theo bảng dưới đây:
...
Ở ví dụ phía dưới `col_2`, `col_3`, `col_4`, `col_5`, `col_6` có thể ép về kiểu `int`, còn `col_6` thì không thể ép kiểu được.
...
```{block2, type='rmdtip'}
**_Mẹo:_**
Nếu dữ liệu có khoảng nhỏ thì thay vì khai báo kiểu `np.int`, ta có thể khai báo `np.int8` để giảm bộ nhớ lưu trữ. 
