...
Dữ liệu JSON với cấu trúc `records` và `index` thường gặp nhất. Cấu trúc `values` thực chất không phải dạng JSON

Với cấu trúc values, khi gọi `pd.read_json()`, kết quả trả về là DataFrame với tên các cột có giá trị 0, 1, 2,...., N-1 với N là số phần tử của từng danh sách con
 ```
#### Xuất file
