...
Khi bạn sử dụng Python version \textgreater= 3.6 và pandas version \textgreater= 0.23 và không truyền biến \texttt{index} vào, index của \texttt{Series} sẽ vẫn giữ nguyên theo thứ tự index của dict truyền vào.

Khi bạn sử dụng pandas version \textless= 0.23 và không truyền biến \texttt{index} vào, \texttt{Series} sẽ đánh index là index của dict được sắp xếp theo thứ tự từ vựng.
\end{rmdnote}
Ví dụ: với bản pandas \textgreater= 0.23, index của \texttt{Series} sẽ là \texttt{{[}"b",\ "a",\ "c"{]}}, còn nếu bạn dùng phiên bản thấp hơn thì index của \texttt{Series} sẽ là \texttt{{[}"a",\ "b",\ "c"{]}}
...
Với các giá trị trong biến \texttt{index} không có trong keys của dict, \texttt{Series} sẽ tạo ra các giá trị bị thiếu \textbf{NaN}.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{In [}\DecValTok{1}\NormalTok{]: pd.Series(\{}\StringTok{"a"}\NormalTok{:}\DecValTok{0}\NormalTok{, }\StringTok{"b"}\NormalTok{:}\DecValTok{1}\NormalTok{, }\StringTok{"c"}\NormalTok{: }\DecValTok{2}\NormalTok{\}, index}\OperatorTok{=}\NormalTok{[}\StringTok{"b"}\NormalTok{, }\StringTok{"c"}\NormalTok{, }\StringTok{"d"}\NormalTok{, }\StringTok{"a"}\NormalTok{])}
\NormalTok{Out[}\DecValTok{1}\NormalTok{]: }
\NormalTok{b    }\DecValTok{1}
\NormalTok{c    }\DecValTok{2}
\NormalTok{d    NaN}
